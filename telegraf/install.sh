...
sudo cp -f telegraf/conf.d/telegraf-node.conf /etc/telegraf/telegraf.d/
if [ "${instance_type}" = 'glusterfs' ]; then
  cp -f telegraf/telegraf-node-glusterfs.conf /etc/telegraf/telegraf.d/
elif [ "${instance_type}" = 'pool' ]; then
  cp -f telegraf/telegraf-pool.conf /etc/telegraf/telegraf.d/
fi
